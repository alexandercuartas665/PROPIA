...
public sealed class WhatsAppLineService : IWhatsAppLineService
{
    ...
    public WhatsAppLineService(PropiaDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    public async Task<IReadOnlyList<WhatsAppLineDto>> ListAsync(CancellationToken ct = default)
    {
        return await _db.WhatsAppLines.AsNoTracking()
            ...
            .Select(l => new WhatsAppLineDto(l.Id, l.InstanceName, l.PhoneNumber, l.Status, l.AssignedToUsuarioTenantId, l.LastConnectedAt, l.LastStatusAt))
            .ToListAsync(ct);
    }
